...
shinyServer(function(input, output) {
    ...
    data <- eventReactive(input$go,{
        ...
        inFile <- input$file
        cleandata = openxlsx::read.xlsx(xlsxFile = inFile$datapath, sheet = 1)
        wq=cleandata[,c("W","Q")]
        wq$W=0.01*wq$W
    ...
    clean <- function(file,advanced=TRUE,includedates=c(1950,as.numeric(format(Sys.Date(), "%Y"))),dummy=NULL,keeprows=NULL,force=NULL,shiny=FALSE,Wmin=NA,Wmax=NA, exclude=TRUE){
      ...
      if(shiny==TRUE){
        ...
        if(type=='text/plain'){
          ...
          observedData=openxlsx::read.xlsx(xlsxFile=datapath,sheet=1)
          names(observedData)=c("Date","Time","Quality","W","Q")
        ...
        if(gsub(".*\\.", "",file)=='txt'){
          ...
          observedData=openxlsx::read.xlsx(xlsxFile=file,sheet=1)
          #added
          names(observedData)=c("Date","Time","Quality","W","Q")
          observedData$Date=as.Date(gsub("\\.","-",observedData$Date),"%d-%m-%Y")
        }else{return(NULL)}
