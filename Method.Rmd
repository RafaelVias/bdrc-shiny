...
\begin{equation}
f(h) = \frac{(x+1)\{ \log A(h) - \log A(1) \}  - x \{ \log P(h) - \log P(1)  \}}{\log h} (\#eq:e4)
\end{equation}
...
\begin{equation}
\log(Q_i) = \log(a) + b \log(h_i -c) + \epsilon_i, \quad i = 1,...,n, (\#eq:e5)
\end{equation}
...
\begin{equation}
\log(Q_{i}) = \log(a) + (b+\beta(h_{i}))\log(h_{i} -c) + \epsilon_{i}, \quad i = 1,...,n, (\#eq:e6)
\end{equation}
